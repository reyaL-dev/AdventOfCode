...
public class Day07PartTwo
{
    ...
    private static long findCombination(long[] numbers, int index, long currentResult, long target)
    {
        ...
        if (resultWithMultiplication != 0)
        {
            return resultWithMultiplication;
        }

        long resultWithConcatenation = findCombination(numbers, index + 1, Long.parseLong(Long.toString(currentResult) + nextNumber), target);
        return resultWithConcatenation;
    }
